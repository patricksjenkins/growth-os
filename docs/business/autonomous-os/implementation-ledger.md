...
|---|---|---|
| Root regression run on implementation branch | Cross-cutting safety | 533 passed, 0 failed locally on 2026-07-24; the repository CI gate also performs secret scanning and real PostgreSQL migration execution. |
...
| `070_scheduling_control.sql`, Calendly adapter, gated webhook projection, and `077_calendly_appointment_projection.sql` | G06 | Adds a tenant-guarded scheduling state machine, policies, events, window validation, owner binding, and collision-resistant idempotency. Strictly verified and exactly allowlisted Calendly receipts can project booking/cancellation state through a service-role-only atomic RPC without canonical PII or raw payloads. The old reply-coordination decision is superseded. Production has no active Calendly integration or scheduling policy; existing Google OAuth flows have mail rather than Calendar scopes. Invitation, reminder, reschedule, preparation, and follow-up execution remain disabled pending provider/calendar activation evidence and consolidated approval. |
...
| Ephemeral PostgreSQL CI harness | G01, G04–G06, G17, G18 and cross-cutting safety | Applies and reapplies migrations 067–077, proves atomic command denial/replay/conflict/cross-tenant behavior, exercises role/classification document RLS, incident reconciliation, closed-won onboarding, and appointment projection; upgrades a simulated released migration-068 database through 077 without freezing legacy rows; and executes containment rollbacks on an evidence-empty upgrade database. CI run 30103607038 passed on 2026-07-24. |
