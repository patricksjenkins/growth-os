...
|---|---|---|
| Root regression run on implementation branch | Cross-cutting safety | 620 passed, 0 failed locally on 2026-07-24; the repository CI gate also performs secret scanning and real PostgreSQL migration execution. |
...
| Ephemeral PostgreSQL CI harness | G01, G04–G06, G15–G19 and cross-cutting safety | Applies and reapplies migrations 067–081, proves atomic command denial/replay/conflict/cross-tenant behavior, exercises role/classification document RLS, ingestion/retrieval citations, incident reconciliation, closed-won onboarding, appointment projection/lifecycle, billing attribution, and monthly finance close; upgrades a simulated released migration-068 database through 081 without freezing legacy rows; and executes evidence-preserving containment rollbacks in reverse dependency order. The 080/081 expansion is pending its first branch CI run; the prior 067–079 workflow was green in run 30104861760. |
