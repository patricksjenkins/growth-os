...
|---|---|---|
| Root regression run on implementation branch | Cross-cutting safety | 524 passed, 0 failed locally on 2026-07-24; the repository CI gate also performs secret scanning and real PostgreSQL migration execution. |
...
| `076_closed_won_onboarding_handoff.sql`, typed planner, and evidence-preserving rollback | G05, G11, G17, G18 | Adds distinct source-sales and client-onboarding tenant identities, authoritative won/customer/workflow checks, one immutable source event, owner acceptance, workflow acknowledgment, SLA/retry/exception states, optimistic revisions, and service-role-only default-off commands. PostgreSQL negative tests cover disabled/authenticated/direct-write/cross-tenant denial, replay, lifecycle advancement, acknowledgment, and completion. Runtime initiation and production activation remain disabled. |
| Ephemeral PostgreSQL CI harness | G01, G04–G06, G17, G18 and cross-cutting safety | Applies and reapplies migrations 067–077, proves atomic command denial/replay/conflict/cross-tenant behavior, exercises role/classification document RLS, incident reconciliation, closed-won onboarding, and appointment projection; upgrades a simulated released migration-068 database through 077 without freezing legacy rows; and executes containment rollbacks on an evidence-empty upgrade database. CI execution for migrations 076–077 is pending branch push. |
