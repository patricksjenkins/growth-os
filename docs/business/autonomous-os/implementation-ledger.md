...
| `069_document_control.sql`, `080_document_ingestion_retrieval.sql`, access/ingestion contracts, and gated read/search API | Document-management requirement, G17, G18 | Adds private canonical metadata, immutable versions and ingestion receipts, tenant-scoped SHA-256 duplicate detection, search chunks with guarded version-specific citations, links, audit events, explicit user/role/agent grants, classification-aware RLS, safe storage-path calculation, and permissioned retrieval. Supabase Storage is the confirmed current provider contract, but provider dispatch is structurally disabled. Upload dispatch, malware/extraction workers, version promotion, signed retrieval, web/mobile UI, evidence observation, and production migration remain gated. |
