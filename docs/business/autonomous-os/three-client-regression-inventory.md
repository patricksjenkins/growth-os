...
| Documents and files | `/api/documents` behind global and exact-tenant cohort flags; canonical control plus default-off ingestion/retrieval contracts | documents, versions, ingestion receipts, chunks, links, events, grants, storage | Supabase Storage; no second cloud adapter is currently configured | No direct client writes; role/classification/grant reads remain tenant-bound; digests, paths, parent links, search, citations, and agent retrieval never cross tenant. | Read/search routes, explicit projections, private storage policy, tenant-scoped SHA-256 deduplication, immutable version/citation evidence, exact user/agent grants, and fabricated-citation rejection are implemented. Provider dispatch, scanning/extraction workers, promotion, signed retrieval, web/mobile UI, and activation remain gated. |
