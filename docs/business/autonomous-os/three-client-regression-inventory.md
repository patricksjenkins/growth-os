...
| Finance and customer attribution | finance/customer routes and workers; shadow canonical calculation contract | finance_entries, customers, jobs, Stripe mapping | Stripe | Existing totals cannot change from code-only work; all reads are tenant-bound; reconciliation differences and missing evidence block authority. | Existing behavior remains unchanged. Exact minor-unit canonical calculations and fail-closed close/signoff controls pass focused tests; tenant adapter, source reconciliation, shadow comparison, and G15 attribution remain gated. |
...
| Scheduling and meetings | legacy Calendly webhook/meetings; canonical scheduling foundation | meetings, appointment workflows/events/policies | No active production calendar provider; Calendly code adapter exists | Legacy bookings and meeting prep remain unchanged; strict verified-event projection is idempotent and exact-tenant gated; no invitation, reminder, or calendar write until authorized. | State machine, PII-minimal Calendly adapter, cross-tenant guards, and atomic booking/cancellation projection implemented. Aggregate production inspection found no active Calendly integration/policy; tenant booking URL and calendar authorization require activation evidence. |
