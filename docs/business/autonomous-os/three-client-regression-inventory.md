...
| Documents and files | new document-control foundation; existing storage paths | documents, versions, chunks, links, events, storage | Existing cloud provider not yet identified | No direct client writes; owner-role reads only; paths and parent links remain tenant-bound; restricted data never crosses tenant. | Ephemeral DB negative tests prepared; routes, ingestion, cloud adapter, scan, UI, and provider identification pending. |
