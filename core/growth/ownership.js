...
const OWNERSHIP = {
  ...
  'prospecting': { category: 'prospecting_growth', owns: 'Wide-net prospect discovery; existing inventory is worked first; 1-9 is preferred and 10-19 accepted', triggers: 'Cron daily 6am ET', reads: 'tenant_config, leads (dedupe)', writes: 'leads + growth_events(discovered)', handoffTo: 'enrichment', channel: null },
  ...
  'facebook-prospecting': { category: 'prospecting_growth', owns: 'Legacy client-tenant Facebook draft workflow; retired from FGA because it requires manual DM coordination', triggers: 'Cron 2pm ET for non-FGA tenants only', reads: 'leads (lifecycle_stage=fb_only)', writes: 'leads (text_message_sent/nurture), conversations (FB DM draft)', handoffTo: 'client owner (manual FB DM)', channel: 'sms' },
  'enrichment': { category: 'prospecting_growth', owns: 'Contact, employee-count, social and website evidence; daily FGA contact recovery for email-missing prospects', triggers: 'Inline in prospecting + 8:00/8:10/13:10/14:10 ET recovery', reads: 'FGA prospect leads and provider evidence', writes: 'leads (email/fb/website, evidence, lifecycle_stage)', handoffTo: 'scoring', channel: null },
  'scoring': { category: 'prospecting_growth', owns: 'Industry-neutral priority score: verified 1-9, estimated 1-9, then 10-19', triggers: 'Cron 7:30am ET daily + event', reads: 'leads (enriched), tenant_config ICP', writes: 'leads + growth_events(qualified)', handoffTo: 'outreach', channel: null },
  'growth-restart': { category: 'prospecting_growth', owns: 'One bounded daily cohort from the reviewed existing-prospect manifest', triggers: 'Cron 7:40am ET daily (FGA-only)', reads: 'restart candidates + customer/suppression/reply evidence', writes: 'one-use restart authorization + outreach draft jobs', handoffTo: 'outreach', channel: null },
  'sequence-recovery': { category: 'prospecting_growth', owns: 'Restore the current seven-touch plan for qualified, provider-proven contacts stranded by legacy retirement', triggers: 'Cron 7:50am ET daily (FGA-only, bounded)', reads: 'contacted FGA prospects + provider receipt + customer/suppression/reply evidence', writes: 'current-plan enrollment only; never sends', handoffTo: 'drip-campaign', channel: null },

  // ── Outreach & Communication ──
  'outreach': { category: 'outreach_comms', owns: 'Personalized first-touch copy for ranked, qualified prospects', triggers: 'Cron daily + accepted handoff', reads: 'estimated/verified 1-19 employee, outreach-ready leads', writes: 'outreach_sequences (draft) + growth_events(drafted)', handoffTo: 'auto-outreach gate', channel: null },
  ...
  'reply-classification': { category: 'outreach_comms', owns: 'Classifies full inbound reply bodies and separates warm interest, questions, objections, no and automation', triggers: 'Cron daily', reads: 'Gmail + conversations (inbound)', writes: 'reply evidence, lead state, suppression, owner handoff', handoffTo: 'owner-handoff / sales-nurture', channel: null },
  'owner-handoff': { category: 'outreach_comms', owns: 'Proves every genuine warm reply has a Patrick-owned next action and 24-hour SLA', triggers: 'Hourly recovery after reply sync (FGA-only)', reads: 'FGA replied/interested lead state and intake safety evidence', writes: 'internal next action, attention receipt and owner notification', handoffTo: 'Patrick → demo', channel: null },
...
const OVERLAP_RULES = [
  ...
  { agents: ['facebook-prospecting', 'drip-campaign'], channel: 'sms/email', resource: 'prospect', owner: 'central gate', rule: 'FGA does not use manual Facebook outreach; enrichment recovers an email before outreach→drip. Legacy client behavior remains tenant-isolated.', severity: 'info' },
